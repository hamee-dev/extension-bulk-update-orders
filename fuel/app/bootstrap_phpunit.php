...
// データプロバイダーをテストよりも先にロードしたいためbootstrapでrequireする
$dataprovider_require = function($dataprovider_path) use (&$dataprovider_require) {
    foreach(glob($dataprovider_path) as $file){
        if(is_file($file) && pathinfo($file, PATHINFO_EXTENSION) === 'php'){
            require_once $file;
        }else if (is_dir($file)) {
            // フォルダがある場合は再帰的にロードする
            $dataprovider_require($file . '/*');
        }
    }
};
$dataprovider_require(APPPATH . 'tests/dataproviders/*');
